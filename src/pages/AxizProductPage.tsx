...
// raw_data carries every original column from Axiz's feed (see axiz-sync.js)
// — these keys are already surfaced elsewhere on the page, so hide them
// from the generic specs table to avoid repeating name/price/images.
const RAW_DATA_HIDDEN_KEYS = new Set([
  'ITEMID', 'ITEMNAME', 'ITEMPRICE', 'AVAILTOSELL', 'Image Url',
  'IMAGEURL2', 'IMAGEURL3', 'IMAGEURL4', 'IMAGEURL5', 'PRODCAT', 'Brand',
]);

export function AxizProductPage() {
  ...
  const seoPlainDescription = product?.short_description
    || (product ? `${product.name} — shop at SPET Online.` : 'Shop the Axiz Digital range at SPET Online.');
  ...
  useSEO({
    ...
    jsonLd: product ? {
      ...
      offers: {
        ...
        url: `https://spetonline.co.za/shop/axiz/product/${product.id}`,
      },
    } : undefined,
  });
  ...
  if (!product) return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a141d]">
      ...
      <div className="text-center py-20">
        ...
        <Link to="/shop/axiz" className="mt-4 inline-block text-orange-600 hover:underline">← Back to Axiz Digital</Link>
      </div>
    </div>
  );
  ...
  const rawSpecs = Object.entries(product.raw_data ?? {}).filter(
    ([k, v]) => !RAW_DATA_HIDDEN_KEYS.has(k) && v !== null && String(v).trim() !== ''
  );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a141d]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-400 mb-8">
          ...
          <span>›</span>
          <Link to="/shop/axiz" className="hover:text-orange-600 transition-colors">Axiz Digital</Link>
          {product.category && <>
            <span>›</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                  ...
                  <button
                    ...
                    className={`flex-shrink-0 w-16 h-16 rounded-xl border-2 overflow-hidden bg-white dark:bg-lago-900 transition-all ${
                      activeImage === img
                        ...
                        : 'border-gray-200 dark:border-lago-700 hover:border-orange-400'
                    }`}
                  >
          ...
          <div>
            {product.brand && (
              <p className="text-xs font-bold text-orange-500 dark:text-orange-400 uppercase tracking-widest mb-2">
                {product.brand}
              </p>
            )}
            ...
            {activeTab === 'description' ? (
              ...
              <table className="w-full text-sm">
                <tbody>
                  ...
                  <tr className={rawSpecs.length % 2 === 0 ? 'bg-gray-50 dark:bg-lago-800/30' : ''}>
