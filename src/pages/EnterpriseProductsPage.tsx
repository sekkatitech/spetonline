...
function StockBadge({ status, qty }: { status: string; qty: number }) {
  if (status === 'out_of_stock' || qty === 0)
    return <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 9999, background: E.redBg, color: E.red }}>Out of stock</span>
  if (qty <= 5 || status === 'low_stock')
    return <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 9999, background: E.amberBg, color: E.amber }}>Low stock ({qty})</span>
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '14px 16px', flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <div style={{
          fontSize: 13, fontWeight: 600, color: E.onSurface,
          lineHeight: 1.4,
          display: '-webkit-box',
          WebkitLineClamp: 2,
        ...
        <div style={{ fontSize: 11, fontFamily: 'monospace', color: E.outline }}>
          SKU: {product.sku}
        </div>

        <div style={{ marginTop: 'auto', paddingTop: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ fontSize: 17, fontWeight: 700, color: E.primary, fontVariantNumeric: 'tabular-nums' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              ...
              style={{
                ...
                border: `1px solid ${E.outlineVariant}`, cursor: 'pointer',
            ...
            <button
              onClick={e => { e.stopPropagation(); if (!outOfStock) onAddToQuote(product) }}
              disabled={outOfStock}
              style={{
                flex: 1,
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                background: outOfStock ? E.surfaceContainer : E.primary,
                color: outOfStock ? E.outline : E.onPrimary,
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <button onClick={onClear} style={{ fontSize: 11, color: E.outline, background: 'none', border: 'none', cursor: 'pointer' }}>
          Clear all
        </button>
      </div>
        ...
        {items.map(item => (
          <div key={item.product.id} style={{
            ...
            <div style={{
              ...
              background: E.surfaceLow, borderRadius: 6, overflow: 'hidden',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}>
              {item.product.image
                ? <img src={item.product.image} alt="" style={{ width: '100%', height: '100%', objectFit: 'contain', padding: 4 }} />
                : <span style={{ fontSize: 16, opacity: 0.3 }}>📦</span>
              }
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 12, fontWeight: 500, color: E.onSurface, lineHeight: 1.3, marginBottom: 6 }}>
                {item.product.name.length > 45 ? item.product.name.slice(0, 45) + '…' : item.product.name}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                {/* Qty controls */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 0, border: `1px solid ${E.outlineVariant}`, borderRadius: 6, overflow: 'hidden' }}>
                  <button
                    onClick={() => onQtyChange(item.product.id, Math.max(1, item.qty - 1))}
                    style={{ width: 24, height: 24, background: E.surfaceLow, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: E.onSurface }}
                  ><MinusIcon /></button>
                ...
                <div style={{ fontSize: 12, fontWeight: 600, color: E.primary, fontVariantNumeric: 'tabular-nums' }}>
                  {formatPrice(item.product.price * item.qty)}
                </div>
              </div>
            </div>
            <button onClick={() => onRemove(item.product.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: E.outline, padding: 2, marginTop: 2 }}>
              <XIcon />
            </button>
          </div>
        ))}
      ...
      <div style={{ padding: '14px 16px', borderTop: `1px solid ${E.outlineVariant}` }}>
        ...
        <div style={{ fontSize: 11, color: E.outline, marginBottom: 12, lineHeight: 1.5 }}>
          Excl. VAT and delivery. Prices valid for 15 business days.
        </div>
        <button
          onClick={onSubmit}
          style={{
...
export default function EnterpriseProductsPage() {
  ...
  const [source, setSource]         = useState<'all' | 'esquire' | 'syntech' | 'enterprise'>('all')
  ...
  const loadProducts = useCallback(async (reset = true) => {
    ...
    try {
      ...
      if (source === 'all' || source === 'esquire') {
        let q = supabase
          .from('products')
          .select('id, ProductName, ProductCode, Brand, Category, Price, AvailableQty, image', { count: 'exact' })
          .eq('is_active', true)
        ...
        if (source === 'esquire') q = q.range(from, to)
        else q = q.range(0, 99) // when 'all', take top 100 from each

        const { data, count } = await q
        if (data) {
          allRows.push(...data.map((p: any) => ({
            id:          p.id,
            name:        p.ProductName,
            sku:         p.ProductCode,
            brand:       p.Brand ?? '—',
            ...
            stockStatus: (p.AvailableQty ?? 0) === 0 ? 'out_of_stock' : (p.AvailableQty ?? 0) <= 5 ? 'low_stock' : 'in_stock',
            image:       p.image,
            source:      'esquire' as const,
            description: null,
          })))
          totalCount += count ?? 0
        }
      ...
      if (source === 'all' || source === 'syntech') {
        let q = supabase
          ...
          .select('id, name, sku, brand, category, price_display, stock_qty, stock_status, thumbnail_url, short_description', { count: 'exact' })
          .eq('is_active', true)
        ...
        if (source === 'syntech') q = q.range(from, to)
        else q = q.range(0, 99)
        ...
        if (data) {
          allRows.push(...data.map((p: any) => ({
            ...
            stock:       p.stock_qty ?? 0,
            stockStatus: (p.stock_status as any) ?? 'out_of_stock',
            image:       p.thumbnail_url,
            source:      'syntech' as const,
  ...
  useEffect(() => {
    loadProducts(true)
  }, [search, source, stockFilter, sortBy])
  ...
  const submitQuote = async () => {
    ...
    try {
      ...
      // Save line items to enterprise_quote_items
      if (quote) {
        await supabase.from('enterprise_quote_items').insert(
          cart.map(i => ({
            quote_id:       quote.id,
            product_id:     i.product.id,
            product_source: i.product.source,
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: E.onSurfaceVariant, marginTop: 1 }}>
              {total.toLocaleString()} products across all ranges
            </p>
          </div>
          {cart.length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: E.onSurfaceVariant }}>
              <CartIcon />
              <span><strong style={{ color: E.primary }}>{cart.length}</strong> items in quote · {formatPrice(cart.reduce((s, i) => s + i.product.price * i.qty, 0))}</span>
            </div>
          )}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 0, flex: 1 }}>
          ...
          <div style={{ padding: '24px 28px', minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
                <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: E.outline }}>
                  <SearchIcon />
                </span>
                <input
                  type="text"
              ...
              {/* Source filter */}
              <div style={{ display: 'flex', gap: 4, background: E.surfaceWhite, border: `1px solid ${E.outlineVariant}`, borderRadius: 8, padding: 4 }}>
                {(['all', 'esquire', 'syntech', 'enterprise'] as const).map(s => (
                  <button key={s} onClick={() => setSource(s)} style={{
                    padding: '5px 12px', borderRadius: 6, border: 'none',
                    background: source === s ? E.primary : 'none',
                    color: source === s ? E.onPrimary : E.onSurfaceVariant,
                    fontSize: 12, fontWeight: source === s ? 600 : 400,
                    cursor: 'pointer', fontFamily: 'inherit',
                    textTransform: 'capitalize',
                  }}>
                    {s === 'all' ? 'All' : s === 'esquire' ? 'Home & Ent.' : s === 'syntech' ? 'Gaming & Computing' : 'Enterprise'}
                  </button>
                ))}
              </div>
              ...
              {/* Stock filter */}
              <button
                onClick={() => setStockFilter(prev => prev === 'all' ? 'in_stock' : 'all')}
                style={{
                  display: 'flex', alignItems: 'center', gap: 6,
                  padding: '8px 14px', borderRadius: 8,
                  border: `1px solid ${stockFilter === 'in_stock' ? E.primary : E.outlineVariant}`,
                  background: stockFilter === 'in_stock' ? E.primary : E.surfaceWhite,
                  color: stockFilter === 'in_stock' ? E.onPrimary : E.onSurfaceVariant,
            ...
            {quoteSuccess && (
              <div style={{
                background: E.greenBg, border: `1px solid ${E.green}`,
                borderRadius: 10, padding: '14px 18px',
                display: 'flex', alignItems: 'center', gap: 12,
                marginBottom: 20, fontSize: 13, color: E.green, fontWeight: 500,
              }}>
                ✓ Your quote has been submitted! Our team will review and respond within 24 hours.
              </div>
            )}
              ...
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 16 }}>
                  {products.map(product => (
                    <ProductCard key={product.id} product={product} onAddToQuote={addToQuote} />
                  ))}
                </div>
                  ...
                  <div style={{ textAlign: 'center', marginTop: 32 }}>
                    <button
                      ...
                      style={{
                        ...
                        color: E.onSurface, cursor: 'pointer',
                        fontFamily: 'inherit',
                      }}
                    >
          ...
          <div style={{
            ...
            borderLeft: `1px solid ${E.outlineVariant}`,
            background: E.surfaceLow,
            ...
            <div style={{ padding: '0 4px' }}>
              ...
              />
              {submitting && (
                <div style={{ textAlign: 'center', fontSize: 12, color: E.onSurfaceVariant, marginTop: 10 }}>
