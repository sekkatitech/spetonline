...
// ── Product card ──────────────────────────────────────────────────────────────
function PinnacleProductCard({ product }: { product: any }) {
...
// ── Main page ─────────────────────────────────────────────────────────────────
export function PinnacleShopPage() {
  useSEO({
    title: 'Pinnacle Products',
    description: 'Shop the Pinnacle range on SPET Online, with fast delivery across South Africa.',
  });
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a141d]">
      ...
      {/* ── Hero banner ── */}
      <div className="bg-gradient-to-br from-teal-800 via-teal-900 to-[#00201a] pb-10 px-4">
        <NavSpacer />
        <div className="max-w-7xl mx-auto">
          <Link to="/shop" className="inline-flex items-center gap-1.5 text-teal-300 hover:text-white text-sm font-medium transition-colors mb-6">
            ← Back to Shop
          </Link>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-4 py-2.5 text-sm bg-white dark:bg-lago-900 border border-gray-300 dark:border-lago-700 text-gray-900 dark:text-white rounded-xl focus:outline-none focus:border-teal-500 w-52"
              />
            </div>
            <button
              onClick={() => setFiltersOpen(!filtersOpen)}
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white dark:bg-lago-900 border border-gray-300 dark:border-lago-700 text-sm font-semibold text-gray-700 dark:text-white hover:border-teal-500 transition-colors shadow-sm"
            >
              <SlidersHorizontal className="w-4 h-4" />
              Filters
              {hasFilters && (
                <span className="w-5 h-5 bg-teal-600 text-white text-[10px] rounded-full flex items-center justify-center">
            ...
            <select
              ...
              className="px-4 py-2.5 rounded-xl bg-white dark:bg-lago-900 border border-gray-300 dark:border-lago-700 text-sm font-semibold text-gray-700 dark:text-white hover:border-teal-500 transition-colors shadow-sm focus:outline-none cursor-pointer"
            >
            ...
            <div>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <button
                  ...
                  className={`relative w-11 h-6 rounded-full transition-colors ${inStockOnly ? 'bg-teal-600' : 'bg-gray-300 dark:bg-lago-700'}`}
                >
                ...
                {categories.map((cat) => (
                  <label key={cat} className="flex items-center gap-2.5 cursor-pointer group">
                    <input type="checkbox" checked={selectedCategories.includes(cat)} onChange={() => toggleCategory(cat)}
                      className="w-4 h-4 rounded text-teal-600 border-gray-300 dark:border-lago-600 bg-white dark:bg-lago-800 focus:ring-teal-500" />
                    <span className="text-sm text-gray-700 dark:text-lago-100 group-hover:text-teal-600 dark:group-hover:text-white transition-colors">{cat}</span>
                  </label>
                ))}
              </div>
            </div>
                ...
                {brands.map((b) => (
                  <label key={b} className="flex items-center gap-2.5 cursor-pointer group">
                    <input type="checkbox" checked={selectedBrands.includes(b)} onChange={() => toggleBrand(b)}
                      className="w-4 h-4 rounded text-teal-600 border-gray-300 dark:border-lago-600 bg-white dark:bg-lago-800 focus:ring-teal-500" />
                    <span className="text-sm text-gray-700 dark:text-lago-100 group-hover:text-teal-600 dark:group-hover:text-white transition-colors">{b}</span>
                  </label>
                ))}
              </div>
            </div>
        ...
        {hasFilters && (
          <div className="flex flex-wrap gap-2 mb-6">
            {selectedBrands.map((b) => (
              <span key={b} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-200 text-sm font-medium">
                {b} <button onClick={() => toggleBrand(b)}><X className="w-3 h-3" /></button>
              </span>
            ))}
            {selectedCategories.map((c) => (
              <span key={c} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-200 text-sm font-medium">
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            {hasFilters && <button onClick={clearAll} className="mt-4 text-teal-600 text-sm hover:underline">Clear all filters</button>}
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {products.map((p) => <PinnacleProductCard key={p.id} product={p} />)}
          </div>
        )}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2 mt-10 flex-wrap">
            <button onClick={() => setPage(1)} disabled={page === 1}
              className="px-4 py-2 rounded-xl bg-white dark:bg-lago-900 border border-gray-300 dark:border-lago-700 text-gray-700 dark:text-white text-sm font-semibold disabled:opacity-30 hover:border-teal-500 transition-colors shadow-sm">
              « First
            </button>
                  ...
                  <button key={p} onClick={() => setPage(p)}
                    className={`w-10 h-10 rounded-xl text-sm font-bold transition-colors shadow-sm ${
                      page === p
                        ...
                        : 'bg-white dark:bg-lago-900 border border-gray-300 dark:border-lago-700 text-gray-700 dark:text-white hover:border-teal-500'
                    }`}>
                    {p}
                  </button>
                );
